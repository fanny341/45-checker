...
zipalign -f -v 4 "$OUTPUT_DIR/gp45_unsigned.apk" "$OUTPUT_DIR/gp45_aligned.apk" > /dev/null 2>&1
...
cp "$OUTPUT_DIR/GP45_v$VERSION.apk" "$OUTFILE" 2>/dev/null || true

rm -rf "$BUILD_DIR"
rm -f "$OUTPUT_DIR/gp45_unsigned.apk" "$OUTPUT_DIR/gp45_aligned.apk"

echo "=== DONE ==="
ls -la "$OUTPUT_DIR/GP45_v$VERSION.apk"
echo "APK: $OUTFILE"
